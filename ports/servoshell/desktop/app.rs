...
impl App {
    ...
    pub fn init(&mut self, active_event_loop: Option<&ActiveEventLoop>) {
        ...
        let running_state = Rc::new(RunningAppState::new(
            ...
            self.preferences
                .dom_gamepad_enabled
                .then(ServoshellGamepadProvider::maybe_new)
                .flatten()
                .map(Rc::new),
        ));
    ...
    fn create_platform_window(
        ...
        {
            let _ = (active_event_loop, url);
            panic!("Nickelium was built without the `headed` feature; run it in headless/agent mode");
        }
    }
