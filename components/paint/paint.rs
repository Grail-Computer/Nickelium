...
pub struct Paint {
    ...
    /// The [`WebGLThreads`] for this renderer, if WebGL is enabled.
    ...
    #[cfg(feature = "webxr")]
    webxr_main_thread: Option<RefCell<webxr::MainThreadRegistry>>,
...
impl Paint {
    pub fn new(state: InitialPaintState) -> Rc<RefCell<Self>> {
        ...
        let should_create_webgl_comm = {
            let mut enabled = pref!(dom_webgl_enabled);
            #[cfg(feature = "webxr")]
            {
                enabled |= pref!(dom_webxr_enabled);
            }
            enabled
        };
        let mut webgl_threads = None;
        #[cfg(feature = "webxr")]
        let mut webxr_layer_grand_manager = None;
        let (swap_chains, busy_webgl_context_map) = if should_create_webgl_comm {
            let WebGLComm {
                webgl_threads: new_webgl_threads,
                swap_chains,
                busy_webgl_context_map,
                #[cfg(feature = "webxr")]
                webxr_layer_grand_manager: new_webxr_layer_grand_manager,
            } = WebGLComm::new(
            ...
            webgl_threads = Some(new_webgl_threads);
            #[cfg(feature = "webxr")]
            {
                webxr_layer_grand_manager = Some(new_webxr_layer_grand_manager);
            }
            (swap_chains, busy_webgl_context_map)
        } else {
            (SwapChains::new(), WebGLContextBusyMap::default())
        };
            ...
            if pref!(dom_webxr_enabled) {
                let mut webxr_main_thread = webxr::MainThreadRegistry::new(
                    ...
                    webxr_layer_grand_manager
                        .take()
                        .expect("WebXR requires WebGL initialization"),
                )
                .expect("Failed to create WebXR device registry");
                state.webxr_registry.register(&mut webxr_main_thread);
                Some(RefCell::new(webxr_main_thread))
            } else {
                None
            }
        };
    ...
    pub fn webxr_running(&self) -> bool {
        #[cfg(feature = "webxr")]
        {
            self.webxr_main_thread
                .as_ref()
                .is_some_and(|webxr_main_thread| webxr_main_thread.borrow().running())
        }
    ...
    #[cfg(feature = "webxr")]
    pub fn webxr_main_thread_registry(&self) -> Option<webxr_api::Registry> {
        self.webxr_main_thread
            .as_ref()
            .map(|webxr_main_thread| webxr_main_thread.borrow().registry())
    }
    ...
    pub fn perform_updates(&self) -> bool {
        ...
        #[cfg(feature = "webxr")]
        if let Some(webxr_main_thread) = self.webxr_main_thread.as_ref() {
            webxr_main_thread.borrow_mut().run_one_frame();
        }
