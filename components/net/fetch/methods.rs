...
fn should_block_visual_subresource(request: &Request) -> bool {
    pref!(network_block_visual_subresources_enabled) &&
...
fn empty_blocked_subresource_response(request: &Request) -> Response {
    let mut response = Response::new(
        request.current_url().clone(),
        ResourceFetchTiming::new(request.timing_type()),
    );
    response.status = HttpStatus::new_raw(StatusCode::NO_CONTENT.as_u16(), vec![]);
    response.url_list = request.url_list.clone();
...
pub async fn main_fetch(
    ...
    if response.is_none() && should_block_visual_subresource(request) {
        trace!(
            "blocking visual subresource {} targeting {:?}",
            request.current_url(),
            request.destination
        );
        response = Some(empty_blocked_subresource_response(request));
    }
